...
class Cx extends TagLib
{
	...
	protected $tags = array(
		...
		'voelse'     => array('attr' => '', 'close' => 0, 'alias' => 'voempty'),
		'foreach'    => array('attr' => 'name,item,key', 'level' => 3),
		'if'         => array('attr' => 'name,condition', 'level' => 2),
		'elseif'     => array('attr' => 'name,condition', 'close' => 0),
	...
	public function _volist($tag, $content)
	{
		...
		$parseStr = $this->parseExpression($name);
		$parseStr .= '<?php if (is_array(' . $name . ')) {';
		if (isset($tag['length']) && '' != $tag['length']) {
			$parseStr .= ' $__LIST__ = array_slice(' . $name . ',' . $this->parseCondition($tag['offset']) . ',' . $this->parseCondition($tag['length']) . ',true);';
		} elseif (isset($tag['offset']) && '' != $tag['offset']) {
			$parseStr .= ' $__LIST__ = array_slice(' . $name . ',' . $this->parseCondition($tag['offset']) . ',null,true);';
		} else {
			$parseStr .= ' $__LIST__ = ' . $name . ';';
		}
		$parseStr .= '} else { $__LIST__=array(); }';

		// DADDY : allow <voelse /> syntax
		$parseStr .= 'if (empty($__LIST__)) { echo "'.$empty.'" ; }';
		$parseStr .= 'if (count($__LIST__) > 0) { $' . $key . ' = 0; ';
	...
	{
		$condition = !empty($tag['name']) ? $tag['name'] : $tag['condition'];
	...
	public function _case($tag, $content)
	{
		...
		if ('$' == $flag) {
			...
			$value = $this->parseCondition(substr($value, 1));
			$value = 'case ' . $value . ': ';
		} elseif (strpos($value, '|')) {
	...
	public function _import($tag, $content, $isFile = false, $type = '')
	{
		...
		if ($isFile) {
			...
			foreach ($array as $val) {
				...
				switch ($type) {
					case 'js':
						$parseStr .= '<script type="text/javascript" src="' . $basepath . '/' . str_replace(array('.', '#'), array(
								'/',
								'.',
	...
	public function _assign($tag, $content)
	{
		...
		if ('$' == $flag) {
			...
			$value = $this->parseCondition(substr($tag['value'], 1));
		} else {
			$value = '"' . $tag['value'] . '"';
		}
