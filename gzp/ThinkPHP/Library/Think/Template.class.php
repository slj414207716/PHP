...
class  Template {
    ...
    public function loadTemplate ($templateFile,$prefix='') {
        if(is_file($templateFile)) {
         ...
         // 根据模版文件名定位缓存文件
	    if (APP_STATUS == 'dev') {
			$encoded = str_replace(array('/', '\\'), '%', str_replace(APP_PATH, '', $templateFile));
	    } else {
		    $encoded = md5($templateFile);
	    }
	    $tmplCacheFile = $this->config['cache_path'] . $prefix . $encoded . $this->config['cache_suffix'];
    ...
    public function parseTagLib($tagLib,&$content,$hide=false) {
		...
		// /<(\w+)((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:\'[^\']*\')|[^>\s]+))?)*)\s*(\/?)>/
		$patternSingleTag = '/$begin$tag((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:\'[^\']*\')|[^$end\s]+))?)*)\s*(?:\/?)$end/is';
		$patternMultiTag  = '/$begin$tag((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:\'[^\']*\')|[^$end\s]+))?)*)\s*$end(.*?)$begin\/$tag\s*$end/is';

		foreach ($tLib->getTags() as $name => $val) {
			$tags = array($name);
			if (isset($val['alias'])) {// 别名设置
				$tags = array_merge($tags, array_filter(array_map('trim', explode(',', $val['alias']))));
			}
			$level    = isset($val['level']) ? $val['level'] : 1;
			$closeTag = isset($val['close']) ? $val['close'] : true;
			foreach ($tags as $tag) {
				// 实际要解析的标签名称
				$parseTag = $hide ? $tag : sprintf('%s:%s', $tagLib, $tag);
				...
				if (!$closeTag) {
					$patterns = str_replace(array('$begin', '$tag', '$end'), array($begin, $parseTag, $end), $patternSingleTag);
					...
					$patterns = str_replace(array('$begin', '$tag', '$end'), array($begin, $parseTag, $end), $patternMultiTag);
					for ($i = 0; $i < $level; $i++) {
						$content = preg_replace_callback($patterns, function($matches) use ($tLib, $tag, $that) {
							return $that->parseXmlTag($tLib, $tag, $matches[1], $matches[2]);
						}, $content);
    ...
    public function parseTag($tagStr){
		...
		if ('\\' == $flag) {
			...
			if ('$' == $flag && '.' != $flag2 && '(' != $flag2) { //解析模板变量 格式 {$varName}
				...
				return '<?php echo ' . $this->parseDotVar($name) . ';?>';
			} elseif ('~' == $flag) { // 执行某个函数
				return '<?php ' . $name . ';?>';
    ...
    public function parseVar($varStr){
        ...
        if(!empty($varStr)){
            ...
            // $Think
            if (false !== strpos($name, '$Think')) {
                $name = preg_replace_callback('/(\$Think\.[a-z0-9_.]+)/is', array($this, 'parseThinkVar'), $name);
            }
            // Object
            if (strpos($name, ':')) {
	 ...
	 * @return array
	 */
	public function parseDotVar($condition) {
		$condition = preg_replace('/\$(\w+):(\w+)\s?/is','$\\1->\\2 ',$condition);
		if (false !== strpos($condition, '$Think')) {
			$condition = preg_replace_callback('/(\$Think\.[a-z0-9_.]+)/is', array($this, 'parseThinkVar'), $condition);
		}
		$condition = preg_replace_callback('/(\$\w+\.[a-z0-9_.]+)/is', function ($m) {
			$tmp = explode('.', $m[0]);
			$var = array_shift($tmp);
			$var_identity = strtolower(C('TMPL_VAR_IDENTIFY'));
			if ($var_identity) {
				$pattern = ($var_identity == 'obj') ? '->%s' : "['%s']";
				foreach ($tmp as &$v) {
					if ($v !== '') {
						$v = sprintf($pattern, $v);
					}
				}
				return sprintf('%s%s', $var, implode('', $tmp));
			} else {
				$tmpArr = $tmpObj = array();
				foreach ($tmp as $v) {
					if ($v !== '') {
				...
				return sprintf('(is_array(%s)?%s%s:%s%s)', $var, $var, implode('', $tmpArr), $var, implode('', $tmpObj));
			}
		}, $condition);
