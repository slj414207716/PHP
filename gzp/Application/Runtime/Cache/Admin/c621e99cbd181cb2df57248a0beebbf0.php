...
<div class="row">
    <div class="col-lg-12">
        <?php  if (is_array($data1)) { $__LIST__ = $data1;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$vi) { $mod = ($i % 2 );++$i; if($_GET['sid'] == $vi['sort']): ?><h1 class="page-header"><?php echo ($vi['name']); ?></h1><?php endif; } } ?>
    </div>
</div>
<div align="right">
    <a href="/Admin/People/add/sid/<?php echo ($sid); ?>"><h3>添加信息</h3> </a>
</div>
            <!--列表-->
            <table cellpadding="0" cellspacing="0" class="table table-striped table-bordered table-hover no-footer">
                ...
                <?php  if (is_array($data)) { $__LIST__ = $data;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$vo) { $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td align="center">
                            <a href="/Admin/People/edit/id/<?php echo ($vo['id']); ?>">修改</a>
                            <a onclick="return confirm('确定要删除吗？');" href="/Admin/People/quit/id/<?php echo ($vo['id']); ?>">删除</a>
                        </td>
                    </tr><?php } } ?>
            </table>
