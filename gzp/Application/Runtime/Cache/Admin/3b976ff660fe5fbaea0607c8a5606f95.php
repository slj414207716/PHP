...
<div id="wrapper">
    ...
    <!--查询-->
    <form id="search" action="/index.php/Admin/Economic/jbzb" method="get">
        <div class="row">
            <div class="col-lg-12">
                    ...
                    <div class="col-lg-12">
                        <label>
                            ...
                            <select name="year">
                                ...
                                <?php  if (is_array($year)) { $__LIST__ = $year;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$v) { $mod = ($i % 2 );++$i;?><option <?php if($_GET['year'] == $v): ?>selected="selected"<?php endif; ?> value="<?php echo ($v); ?>"><?php echo ($v); ?>年</option><?php } } ?>
                            </select>
                        </label>
                        <label>
                            项目
                            <select name="sid">
                                <option <?php if($_GET['sid'] == 0): ?>selected="selected"<?php endif; ?> value="0">--请选择--</option>
                                <option <?php if($_GET['sid'] == 1): ?>selected="selected"<?php endif; ?> value="1">地区生产总值</option>
                                <option <?php if($_GET['sid'] == 2): ?>selected="selected"<?php endif; ?> value="2">财政收入</option>
                                <option <?php if($_GET['sid'] == 3): ?>selected="selected"<?php endif; ?> value="3">规上工业总产值</option>
                                <option <?php if($_GET['sid'] == 4): ?>selected="selected"<?php endif; ?> value="4">固定资产投资</option>
                                <option <?php if($_GET['sid'] == 5): ?>selected="selected"<?php endif; ?> value="5">社会消费品零售额</option>
                                <option <?php if($_GET['sid'] == 6): ?>selected="selected"<?php endif; ?> value="6">使用内资</option>
                                <option <?php if($_GET['sid'] == 7): ?>selected="selected"<?php endif; ?> value="7">引进外资</option>
                            </select>
                        </label>
    ...
    <form action="" method="post">
        <table cellpadding="0" cellspacing="0" class="table table-striped table-bordered table-hover no-footer">
            ...
            <?php  if (is_array($shxfData)) { $__LIST__ = $shxfData;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$v) { $mod = ($i % 2 );++$i;?><tr>
                ...
                <td align="center"><?php echo ($v['month']); ?></td>
                <td align="center">
                    <?php if($v['sid'] == 1): ?>地区生产总值
                    <?php elseif($v['sid'] == 2): ?>
                        财政收入
                    <?php elseif($v['sid'] == 3): ?>
                        规上工业
                ...
                <td align="center"><?php echo ($v['plan']); ?></td>
                <td align="center"><?php echo ($v['real']); ?></td>
                <td align="center">
                    <a href="/index.php/Admin/Economic/jbzbEdit/id/<?php echo ($v['id']); ?>">修改</a> |
                    <a onclick="return confirm('确定要删除吗？');" href="/index.php/Admin/Economic/jbzbDel/id/<?php echo ($v['id']); ?>">删除</a>
                </td>
            </tr><?php } } ?>
        </table>
    </form>
