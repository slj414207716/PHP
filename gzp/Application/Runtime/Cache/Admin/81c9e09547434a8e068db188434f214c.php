...
<!--查询-->
<form id="search" action="/index.php/Admin/User/lst" method="get">
    <div class="row">
        <div class="col-lg-12">
            <div class="form-group row">
                <div class="col-lg-12">
                    <label>
                        用户名
                        <input name="username" type="text" value="<?php echo ($_GET['username']); ?>" class="form-control-inline">
                    </label>
                    <button type="submit" class="btn btn-sm btn-primary">查询</button>
                    <a href="add" class="btn btn-sm btn-primary pull-right">添加</a>
                </div>
            </div>
        </div>
    </div>
...
<table cellpadding="0" cellspacing="0" class="table table-striped table-bordered table-hover no-footer">
    <tr>
        <th width="10%" scope="col">序列</th>
        <th width="25%" scope="col">用户名</th>
        <th width="25%" scope="col">最后登录时间</th>
        <th width="30%" scope="col">角色</th>
        <th width="10%" scope="col">编辑</th>
    </tr>
    <?php  if (is_array($data)) { $__LIST__ = $data;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$v) { $mod = ($i % 2 );++$i;?><tr>
            ...
            <td align="center">
                <a href="edit/id/<?php echo ($v['user_id']); ?>">修改</a> |
                <a onclick="return confirm('确定要删除吗？');" href="delete/id/<?php echo ($v['user_id']); ?>">删除</a>
            </td>
        </tr><?php } } ?>
</table>
