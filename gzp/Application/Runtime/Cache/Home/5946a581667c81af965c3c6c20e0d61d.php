...
<h1 class="masked">
	...
	<span>综治维稳 / 综治办社会治安防控体系建设工作基础数据（人防体系）</span>
</h1>
...
<div id="outer">
	...
	<div id="content">
		<ul style="display:block;">
			...
			<div class="aqsc">
				<h3>综治办社会治安防控体系建设工作基础数据（人防体系）</h3>

				<div class="search_2">
					<form action="" method="get" class="sys_form">
						<label>按年份筛选</label>
						<select name="start_year">
							<?php $__FOR_START_18089__=$current_year-5;$__FOR_END_18089__=$current_year+1;for($i=$__FOR_START_18089__;$i < $__FOR_END_18089__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?>年</option><?php } ?>
						</select>
						<select name="start_month">
							<?php $__FOR_START_17454__=1;$__FOR_END_17454__=13;for($i=$__FOR_START_17454__;$i < $__FOR_END_17454__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?>月</option><?php } ?>
						</select>
						&nbsp;&nbsp; — &nbsp;&nbsp;
						<select name="end_year">
							<?php $__FOR_START_15311__=$current_year-5;$__FOR_END_15311__=$current_year+1;for($i=$__FOR_START_15311__;$i < $__FOR_END_15311__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?>年</option><?php } ?>
						</select>
						<select name="end_month">
							<?php $__FOR_START_14684__=1;$__FOR_END_14684__=13;for($i=$__FOR_START_14684__;$i < $__FOR_END_14684__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?>月</option><?php } ?>
						</select>
		...
		<tr>
			...
			<td>
				<ul class="page_button_group">
					<li>
						<a href="/index.php/Home/Treat/basicrf?<?php echo ($PAGE['page_button_prm']); ?>now_page=<?php echo ($PAGE['first_page']); ?>">首页</a>
					</li>
					<li>
						<?php if( $PAGE['prev_page'] == 'disable' ): ?><a href="javascript:void(0);" class="disable">上一页</a>
						<?php else: ?>
							<a href="/index.php/Home/Treat/basicrf?<?php echo ($PAGE['page_button_prm']); ?>now_page=<?php echo ($PAGE['prev_page']); ?>">上一页</a><?php endif; ?>
					</li>
					<?php  if (is_array($PAGE['page_button'])) { $__LIST__ = $PAGE['page_button'];} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$vo) { $mod = ($i % 2 );++$i;?><li>
						<a href="/index.php/Home/Treat/basicrf?<?php echo ($PAGE['page_button_prm']); ?>now_page=<?php echo ($vo); ?>" id="page_<?php echo ($vo); ?>"><?php echo ($vo); ?></a>
					</li><?php } } ?>
					<li>
						<?php if( $PAGE['next_page'] == 'disable' ): ?><a href="javascript:void(0);" class="disable">下一页</a>
						<?php else: ?>
							<a href="/index.php/Home/Treat/basicrf?<?php echo ($PAGE['page_button_prm']); ?>now_page=<?php echo ($PAGE['next_page']); ?>">下一页</a><?php endif; ?>
					</li>
					<li>
						<a href="/index.php/Home/Treat/basicrf?<?php echo ($PAGE['page_button_prm']); ?>now_page=<?php echo ($PAGE['end_page']); ?>">尾页</a>
					</li>
				</ul>
			</td>
			<td>
