...
<div id="outer">
    <ul id="tab">
    	<?php  if (is_array($data3)) { $__LIST__ = $data3;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$vi) { $mod = ($i % 2 );++$i;?><li title="<?php echo ($vi['name']); ?>" num="<?php echo ($vi['sort']); ?>" <?php if($_GET['sid'] == $vi['sort']): ?>class="current"<?php endif; ?> ><?php echo (msubstr($vi['name'],0,8,'utf-8')); ?></li><?php } } ?>
    </ul>
    <div id="content">
<!--社会化管理人员情况-->
		<div class="data05">
	...
	<?php  if (is_array($data3)) { $__LIST__ = $data3;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$ve) { $mod = ($i % 2 );++$i;?><ul <?php if($_GET['sid'] == $ve['sort']): ?>style="display: block;" <?php else: ?> style="display: none;"<?php endif; ?> >
        	...
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php  if (is_array($data)) { $__LIST__ = $data;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$vo) { $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php echo sprintf('%.2f%%',$vo['number']/$dataSum*100);?></td>
				</tr><?php } } ?>
				<tr>
					<th scope="col">合计</th>
					<th scope="col" colspan="2">
						全年总人数：<strong><span class="hj"><?php echo ($dataSum); ?></span></strong>&nbsp人
					</th>
					<th><a class="search1" href="<?php echo (HOST_NAME); ?>/Home/sports/yearWhty/sid/<?php echo ($ve['sort']); ?>">年度数据查询</a></th>
				</tr>
			</table>
        </ul><?php } } ?>
	...
	$(function(){
		window.onload = function()
		{
			...
			$li.click(function(){
				...
				console.log(url);
				window.location.href= url+"/Home/sports/whty/sid/"+num;
				/*
				var $this = $(this);
				var $t = $this.index();
				$li.removeClass();
