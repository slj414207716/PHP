...
<div class="general">
    <?php  if (is_array($data)) { $__LIST__ = $data;} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$v) { $mod = ($i % 2 );++$i;?><ul>
        <h1><?php echo ($v['name']); ?></h1>
        <?php  if (is_array($v['child'])) { $__LIST__ = $v['child'];} else { $__LIST__=array(); }if (empty($__LIST__)) { echo "" ; }if (count($__LIST__) > 0) { $i = 0; foreach ($__LIST__ as $key=>$v1) { $mod = ($i % 2 );++$i;?><li>
                <!--
                <img src="/Public/Home/Static/images/timg.gif">
                -->
            <a href="<?php echo (HOST_NAME); ?>/<?php echo (MODULE_NAME); ?>/<?php echo ($v1['controller']); ?>/<?php echo ($v1['action']); ?>/sid/<?php echo ($v1['sid']); ?>">
           <!--  <a href="<?php echo (HOST_NAME); ?>/<?php echo (MODULE_NAME); ?>/<?php echo (CONTROLLER_NAME); ?>/<?php echo ($v1['action']); ?>/sid/<?php echo ($v1['sid']); ?>"> -->
                <i class="fa fa-<?php echo ($v1['logo']); ?> fa-2x"></i><p><?php echo ($v1['name']); ?></p>
            </a>
            </li><?php } } ?>
    </ul><?php } } ?>
</div>
